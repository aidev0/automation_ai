...
def run_inference(messages: list[dict], model_name: str) -> str:
    ...
    Args:
        ...
        model_name: The specific API identifier for the LLM model
                    (e.g., "gemini-1.5-pro-latest", "gpt-4o", "claude-3-opus-20240229").
    ...
    try:
        if provider == "google":
            ...
            response = client.messages.create(
                model=model_name,
                max_tokens=2048, # You might want to make this configurable
...
def test_llm_models():
    ...
    #     try:
    #         print(f"\n--- Testing Anthropic Claude ({test_model_anthropic}) ---")
    #         response_anthropic = run_inference(sample_messages_with_system_and_history, test_model_anthropic)
    #         print(f"Anthropic Claude Response: {response_anthropic}")
    #     except Exception as e:
    #         print(f"Error during Anthropic Claude test: {e}")
    # else:
    #     print("\nSkipping Anthropic Claude tests: ANTHROPIC_API_KEY not set.")
